...
local window_state_mod = require("workspace_manager.session.window_state")
...
function M.restore_workspace(workspace_state, opts)
  ...
  for i, window_state in ipairs(workspace_state.window_states) do
    if i == 1 and opts.window then
      if opts.resize_window == true or opts.resize_window == nil then
        ...
        local pw = window_state.window_pixel_width
          or window_state.size.pixel_width
        local ph = window_state.window_pixel_height
          or window_state.size.pixel_height
      ...
      opts.tab, opts.pane, opts.window =
        wezterm.mux.spawn_window(spawn_window_args)
    end
  ...
  for i, window_state in ipairs(workspace_state.window_states) do
    if window_state.is_focused and restored_windows[i] then
      local focus_ok, gui_win = pcall(
        function() return restored_windows[i]:gui_window() end
      )
      if focus_ok and gui_win then gui_win:focus() end
      break
    end
  end
end
...
function M.get_workspace_state()
  return M.get_workspace_state_for(wezterm.mux.get_active_workspace())
end

---Captures the state of a workspace by name.
---@param workspace_name string
---@return WorkspaceManagerWorkspaceState
function M.get_workspace_state_for(workspace_name)
