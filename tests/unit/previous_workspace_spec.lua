...
describe("previous workspace tracking", function()
  ...
  it(
    "returns from A to B after switching through C without focus events",
    function()
      ctx.next_workspace(ctx.window, ctx.pane)
      ctx.next_workspace(ctx.window, ctx.pane)
      check_pair("C", "B")
  ...
  it(
    "tracks every live, saved, custom, path, and new switcher route",
    function()
      ...
      for _, mode in ipairs({ "new", "new_at_path" }) do
        ...
        ctx.actions
          .switcher_keymap("n", "CTRL", mode)
          .action(ctx.window, ctx.pane)
  ...
  it(
    "preserves the target after cancellation, reselection, and failure",
    function()
      ctx.next_workspace(ctx.window, ctx.pane)
      ctx.actions.workspace_switcher()(ctx.window, ctx.pane)
  ...
  it(
    "does not rewind history when startup restoration finishes late",
    function()
